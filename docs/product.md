...
현재 자동 브라우저 대상은 Chromium이다. 실제 클립 H.264/AAC 재생을 확인했고, 프레임 API가 없는 환경과 미디어 오류는 오류 상태와 새로고침 버튼으로 알린다. 다른 브라우저·모바일 터치 조작 지원은 아직 합의되지 않았다. 로컬 사용 후 안내 숨김 대응과 추가 입력 감소를 살피되 학습 효과를 수치로 입증했다고 주장하지 않는다.
...
사용자 피드백에 따라 현재 여섯 서비스 영상의 오디오에 신호 gain 0.33(약 −9.63dB)을 한 번 적용했다. 플레이어 볼륨 설정은 유지하며 체감 음량이 정확히 1/3이라는 뜻은 아니다. H.264 화면 스트림과 모든 디코딩 프레임·타임스탬프, 오디오 패킷 시각·길이를 보존하고 AAC 오디오만 다시 인코딩했다. 원시 실험 녹화·입력 증거와 판정 구간은 변경하지 않았다. 파일명에 `-audio033`을 붙여 이전 영상 캐시와 구분한다. [변환 기록](evidence/service-audio-gain-20260923.json)에 입력/출력 해시·gain·신호 검증을 남겼으며, 후속 변환 때 이미 감쇠된 출력에 같은 조정을 중복 적용하지 않는다. 현행 서비스 미디어 정렬 기록은 새 파일명·해시를 참조한다.
