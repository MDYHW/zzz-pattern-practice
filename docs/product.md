...
누구나 접속하는 웹사이트에서 고정된 제어스킬 대응을 반복훈련한다. 베스퍼 두 패턴, 기르타블리르·퇴행 변종, 쿠사리쿠, 환영의 화살 유닛, 태양의 잔불 기본·융합형과 태초의 악몽·창조주, 피의 청소부의 총 아홉 패턴을 제공한다. 쿠사리쿠와 환영의 화살 유닛의 확장·정리는 종료했으며, 환영의 화살 유닛은 마지막 SPACE/RMB 선택과 겹침 레인을 포함한다. 태양의 잔불은9월24일 실험 결과에 따라 기본 진입+3타·준비RMB1회 비채점 안내와 융합 진입+4타를 별도 영상으로 채택했다. [현행 조건·구간](experiments/sun-control-skill.md#실험-결과와-서비스-채택--2026-09-24)을 따른다. 원격 공개와 미측정 대체 입력의 추가는 별도 합의를 따른다. 성공 대응 영상과 선택적으로 표시하는 리듬 안내를 통해, 조작점수를 얻는 데 필요한 대응 순서와 타이밍을 익히게 한다.
...
### 제어스킬과 일반 패턴의 구분 — 2026-09-24

제어스킬은 **보라색 십자가로 시작하고 이후 공격에는 개별 대응 신호가 없어**, 순서와 리듬을 미리 익힐 필요가 있다. 일반 패턴은 공격에 노란색 또는 빨간색 십자가 신호가 있다. 이 플레이 경험의 차이를 먼저 확인하며, 내부 `Execute`·Pre/Combo·QTE 설정의 존재만으로 제어스킬로 분류하지 않는다. 일반 패턴도 연속 수행의 난도와 학습 가치가 충분하면 별도 확장 대상이 될 수 있다.

사용자가 영상을 확인해 **기르타블리르 퇴행 변종에는 제어스킬이 있고, 기본·융합 기르타블리르에는 없다고 확정**했다. 기본·융합형은 엽석연과 같은 일반 패턴 조사 범주이며, 패턴이 단순하고 난도가 낮아 이번 확장 대상에서 모두 제외한다. 내부의 미해독 설정을 이유로 이 결정을 미확인·보류 후보로 되돌리지 않는다.

---

## 피의 청소부 제어스킬 추가 — 2026-09-24

아리아→남궁우→수나·아리아 시작의 측정30회와 준비 극한 회피에 성공한 대표 녹화로 진입RMB+Space5를 제공한다. 원본0–1135프레임(끝 제외), 최초 문구322프레임을 공통 원점으로 쓴다. 준비RMB1회와 앞선 위치 조정·준비 W는 비채점 안내이며 결과는 본6타다. 3타 늦음·4타 이른쪽·5타 늦음의25ms 서비스 여유는 사용자 승인에 따른 미검증 추가 구간이고 실험기의50ms 격자 측정값은 보존한다. [측정·채택 정본](experiments/graymane-control-skill.md)과 [서비스 자산](evidence/graymane-service-media-20260924.json)을 따른다. 기존 화면·입력·마스크 구조를 사용한다.

## 창조주 제어스킬 추가 — 2026-09-24

강습전 일반 태초의 악몽·창조주 Execute_01을 별도 보스로 제공한다. 엽빛나→다이아린→수나·엽빛나 시작의 측정28회와 별도 기준 성공 녹화를 근거로 진입RMB+Space4를 채택했다. 준비 회피2회·W 이동은 비채점 영상 경로 안내이며 준비 종료 후 본 대응만 결과에 포함한다. 문구 최초 일치715프레임을 원점으로 사용한다. 4타9300ms의 성공·실패 혼재와 대표 영상에 대응하는 raw 미확보는 [창조주 정본](experiments/vessel-control-skill.md)에 보존한다. 미측정 진입Space·마지막RMB는 제공하지 않는다. 화면·마스크·모바일 입력은 기존 구조를 사용한다.

## 일반 패턴 확장 선정 — 2026-09-24

사용자가 제어스킬 아홉 패턴의 확장 단계를 종료했다. 다음 확장은 아래 일반 패턴 범위로 진행하며, 종료한 보스의 승인된 서비스 여유에 대한 추가 실게임 확인은 이 단계의 선행 조건이 아니다.

제어스킬 외에도 수행 난도가 높고 준비 전투 없이 분리해 반복할 가치가 있는 패턴을 조사한다. 제어스킬은 건너뛸 수 없고 조작점수를 위해 완벽 대응이 필요하다는 기존 동기가 있다. 일반 패턴은 대응 이득이 있다는 사실만으로 채택하지 않으며, 어려운 연속 수행을 익힐 가치와 이후 전투·사이클에 주는 이득을 함께 판단한다. 기존 경험과의 중복·실제 등장 빈도도 보되 구현 용이성·내부 타수·명목 폭으로 학습 가치를 대신하지 않는다.

사용자는 엽석연, 일반 아바루스, 미아즈믹·도플갱어의 코마노 마나토·제인·「변절자」를 1순위 조사 대상으로 합의했다. 일반 패턴 조사 task에서 탈피 아바루스의 3회 패링 대신 일반 아바루스의 구르기→점프→연속 내려찍기로 교체하는 안을 승인했으며, 탈피형의 출현 빈도를 일반형에 복사하지 않는다. 보스 선정은 구체적 패턴이나 서비스 채택의 확정이 아니다. 각 대상의 연습할 연속 동작·범위·필요 입력을 외부 영상과 내부 구현에서 선별하며, 마지막 성공 처리에 전체 패링 누적 조건이 없다는 이유만으로 앞선 연속 수행의 학습 가치를 배제하지 않는다. 조사 담당과 현재 결과 경로는 README를 따른다. 태양의 잔불 확장은 종료했으며 이 사전 조사와 별도로 관리한다.

보스당 패턴 수를 하나로 제한하지 않는다. 선정 단위는 실제 전투에서 시작 전조부터 후속 공격·마무리까지 이어지는 연속 수행이다. 내부 상태·키 종류가 달라도 하나의 수행이면 묶으며, 중간 대기·쉬운 동작도 다음 대응 준비에 필요하면 포함한다. 전반 입력이 미확인이라는 이유로 후반만 콘텐츠로 잘라내지 않는다. 반대로 서로 독립된 연계에 별도 학습 가치가 있으면 같은 보스에서 여러 패턴을 제안한다. ‘주 후보’는 조사 순서이며 개수 제한이 아니다. 후보 수와 서비스 추가 확정 수를 구분하고, 전체 수행의 난도·이득으로 선정한다.
...
태양의 잔불 융합형도9월24일 사용자 체감 요청에 따라 진입의 웹 판정만 양끝25ms씩 확대했다. 실측 근거·실험 프로필은 유지하며 [태양의 잔불 정본의 서비스 조정](experiments/sun-control-skill.md#공개-후-융합-진입-판정-조정--2026-09-24)을 따른다.
...
- R4. 사용자는 리듬 안내를 숨기고 확대된 영상으로 연습할 수 있다. 녹화된 키보드·마우스 입력 표시는 기본적으로 가리며, 인게임 입력 가리기와 독립적으로 전환한다. 모바일에서는 원본 HUD 입력 영역을 항상 가리고 터치 버튼의 영상/고정 아이콘을 전환한다. [모바일 터치 UI](#모바일-터치-ui--2026-09-23)를 따른다. 표시 설정은 재시도에도 유지하고 진행 중 변경해도 영상 요소·시각·입력 판정은 유지한다.
...
| D2. 추가 입력·공격 사이 입력 | 연습 진행 중 수집한 독립 입력 중, 아직 성공하지 않은 대응을 성공시킨 1회만 소비하고 나머지는 전체 시도의 추가 입력으로 센다. 공격 사이 입력을 앞·뒤 공격에 억지로 배정하지 않는다. | 조기·오답·성공 후 중복 입력을 일관되게 집계한다. 공격별 성공/실패와 시도 전체 추가 입력을 분리해 종료 결과에서 확인한다. 이른 입력 뒤 정답으로 회복해도 추가 1회는 남는다. |
...
- **흐름:** 선택·준비 → 시작 → 성공 영상·안내와 입력 판정 → 실수가 있어도 모든 대응 진행 → 전체 완수·각 대응·추가 입력 → 수동 재시도. 전체 완수는 진입을 포함한 모든 대응 성공이며 추가 입력이 있어도 취소하지 않는다. 안내를 숨겨도 같은 판정이다.
...
현재 자동 브라우저 대상은 Chromium이다. 실제 클립 H.264/AAC 재생을 확인했고, 프레임 API가 없는 환경과 미디어 오류는 오류 상태와 새로고침 버튼으로 알린다. 모바일 터치는 아래 구현 범위로 승인·배포되었으며 실제 모바일 브라우저 검증은 남아 있다. 로컬 사용 후 안내 숨김 대응과 추가 입력 감소를 살피되 학습 효과를 수치로 입증했다고 주장하지 않는다.
...
상단 메뉴는 기본 숨김을 유지하며 화면 전체 너비와 메뉴 높이 영역에 포인터가 들어오면 즉시 플로팅으로 표시한다. 메뉴 밖으로 포인터와 키보드 포커스가 모두 나가면 120ms 후 숨긴다. 기존 스크롤 접근과 Escape 닫기를 유지한다. 리듬 바 오른쪽 세로 버튼으로 접는다. 데스크톱·모바일 가로 화면의 펼치기 버튼은 영상 오른쪽 바깥 면 하단에 두며, 왼쪽 강조선과 오른쪽 둥근 모서리로 접기 버튼과 통일한다. 모바일 세로 배치는 아래 모바일 터치 UI 절을 따른다. 연습 중 영상 위의 구간 밖·선택과 다른 입력 문구를 제거하며 판정·타일 결과는 유지한다. 이 후속 수정은 모바일 터치 기능과 함께 9월23일 공개 배포했다.
...
## 모바일 터치 UI — 2026-09-23

사용자 승인으로 가로 화면을 우선하는 터치 UI를 구현하고 9월23일 공개 배포했다. 터치 기기 또는 `?mobile=1`에서 활성화하며, 개발 서버의 `/mobile-preview.html`에서 가로844×390·667×375와 세로390×844를 전환해 확인한다. 이 페이지는 Vite 기본 production 빌드의 진입점이 아니다.
...
누르는 순간 기존 영상 시각 판정에 연결하며 길게 누르기·호환 click은 중복 제출하지 않는다. 같은 동작을 여러 손가락으로 누르면 놓을 때까지 한 번만 입력하며, 서로 다른 두 동작은 각각 입력한다. 취소·포커스 이탈·일시정지 시 접촉 상태를 해제한다. 준비/정지 중 버튼은 비활성화하고 종료 시 제거한다. 메뉴는 명시적인 열기/닫기 버튼을 제공하며 열면 일시정지한다. 세로 화면은 별도 버튼 줄 없이 HUD 폭27.5% 안에 두 버튼을 축소 배치한다(390px 화면에서 영상378px·버튼약48px). 패드 상단은 큰 Q 아이콘의 위쪽 끝(1280×720 원본 y=525) 아래로 제한한다. 세로 결과는 화면 하단 대신 실제 연습 영역 높이를 기준으로 리듬 바 하단에 정렬한다. 세로 배치 조정도 사용자 승인으로 9월23일 공개 배포했다. 세로 화면의 접기는 리듬 바 안쪽 오른쪽의20px 세로 버튼으로, 펼치기는 영상 바로 아래의 전체 폭·높이24px 가로 버튼으로 제공한다. 가로 화면의 펼치기는 영상 오른쪽 바깥 하단에 둔다. 세로 화면은 좌우6px 여백만 남겨 영상과 리듬 바를 넓히고, 상단 메뉴의 보스/스킬 선택은2열로, 리듬 안내·녹화 입력 가리기·인게임 입력 가리기 설정은3열 한 행으로 배치한다. 영상 가림 면적을 줄이기 위해 세로 메뉴 버튼 높이28px, 선택 글자11px·설정 글자10px, 행 간격4px로 압축한다. 세로 플로팅 메뉴 폭은 최대320px로 줄이고, 버튼 상태는 공통으로 ON/OFF로 표시한다. 세로 메뉴 상태 표기는 버튼 이름 아래 고정 폭으로 가운데 정렬한다. 모바일 종료 조작 화면은 기본 결과 도표 위의 남은 영상 영역 전체를 채우며, 조작 버튼은 그 공간의 가운데에 둔다. 기본 도표 높이를 기준으로 하므로 상세 결과를 확장해도 버튼은 이동하지 않는다. 모바일 도표는 150px 높이·64/110px 입력 행과 상하 2px 패딩으로 압축하며, 844×390에서 기본 결과 높이는 154px다. 모바일 리듬 바는 Space·우클릭 설명을 숨기고 아이콘34px을 유지하며 일반80px, 준비 안내가 있는 쿠사리쿠96px로 압축한다. 가로 시간축과 판정 구간은 유지한다. 모바일 결과는 상단 성공/추가 입력 요약과 범례·행별 추가 입력 수를 시각적으로 숨겨 도표로 전달하며, 접근성 요약과 아이콘 선택 상세는 유지한다.
